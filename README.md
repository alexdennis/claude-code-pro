...
2. **Set a verifiable end state and walk away.** `/goal` keeps a session running
   ...
   Conditions like *"`npm test` exits 0 and p95 stays under 300ms"* work; *"make it
   fast"* does not.
...
Source of the ideas: Addy Osmani's *Claude Code Pro-Tips*. Command behaviour here
...
3. Before building anything fuzzy, ask for a **blind-spot pass**: *"what am I not
   accounting for here? Ask me one question at a time."*
4. Don't accept the first answer. *"Prove this works." "Grill me on this design."
   "Now do the elegant version."*
